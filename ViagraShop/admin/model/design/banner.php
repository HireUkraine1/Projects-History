	...
	public function addBanner($data) {
		...
		if (isset($data['banner_image'])) {
			foreach ($data['banner_image'] as $banner_image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "banner_image SET banner_id = '" . (int)$banner_id . "', link = '" .  $this->db->escape($banner_image['link']) . "', desc1 = '" .  $banner_image['desc1'] . "', sort_order = '" .  $this->db->escape($banner_image['sort_order']) . "', image = '" .  $this->db->escape($banner_image['image']) . "'");
