...
function simple_login_open() {
    ...
    jQuery('#simple_login_layer').fadeTo(500,0.3);
    return false;
}
...
jQuery(function(){
    ...
    jQuery(document).on('change', 'input[reload]:not([autocomplete]):not([googleapi]),select[reload],textarea[reload]', function(){
    ...
    jQuery(document).on('keydown', '#simplecheckout_form input, #simplecheckout_form textarea', function(){
    ...
    jQuery(document).on('change', '#simplecheckout_form input[type=radio]:not([reload]),#simplecheckout_form input[type=checkbox]:not([reload]),#simplecheckout_form select:not([reload])', function(){
        if (jQuery(this).parents('#simplecheckout_payment_form').length) {
            return;
        }
        customer_field_changed();
    ...
    jQuery(document).on('change', '#checkout_customer_main_email', function(){
    ...
    jQuery(document).on('change', '#email_confirm', function(){
    ...
    jQuery(document).on('click', 'table.cart td.remove img,.mini-cart-info td.remove img,table.s_cart_items a.s_button_remove', function(){
