...
class LitExtension_CartMigration_Model_Cart_Loadedcommercev6 extends LitExtension_CartMigration_Model_Cart {
    ...
    public function convertCategory($category, $categoriesExt) {
        ...
        if ($category['parent_id'] == 0 || $category['parent_id'] == $category['categories_id']) {
            preg_match("/AM:(.+)/", $cat_data['name'], $match);
            $cat_parent_id = 3;
            if (!$match) {
                preg_match("/MZ:(.+)/", $cat_data['name'], $match);
                $cat_parent_id = 2;
            }
            if (!$match) {
                preg_match("/AK:(.+)/", $cat_data['name'], $match);
                $cat_parent_id = 4;
            }
            if (!$match) {
                return array(
                    'result' => 'warning',
                    'msg' => $this->consoleWarning("Category Id = {$category['categories_id']} import failed. Error: Category is not belong to any site!")
                );
            }
            $cat_data['name'] = trim($match[1]);
    ...
    protected function _convertProduct($product, $productsExt, $parent_id = null) {
            ...
            if ($proCat) {
                foreach ($proCat as $pro_cat) {
                    ...
                    if ($cat_id) {
                        ...
                        $websiteIds = array_merge($websiteIds, array_unique($stores->getColumnValues('website_id')));
                    }
                }
            }
        ...
        $pro_data['website_ids'] = $websiteIds;
        $stores = Mage::getModel('core/store')->getCollection()
                                ->addWebsiteFilter($websiteIds);
        $pro_data['store_ids'] = array_unique($stores->getColumnValues('store_id'));
